...
inspectors and must not depend on Linspector.  Suite Modules enhance the host
...
class ModuleContext:
    ...
    def add_configuration_page(self, page_id, label, factory):
        ...
        if self._add_configuration_page_cb is None:
            raise RuntimeError("This Linspector Suite host does not support Configuration page contributions.")
    ...
    def open_configuration_page(self, page_id):
        ...
        if self._open_configuration_page_cb is None:
            raise RuntimeError("This Linspector Suite host does not support Configuration page navigation.")
    ...
    def add_suite_state_item(self, label, provider):
        ...
        if self._add_suite_state_item_cb is None:
            raise RuntimeError("This Linspector Suite host does not support Suite State contributions.")
    ...
    def add_coverage_annotation(self, provider):
        ...
        if self._add_coverage_annotation_cb is None:
            raise RuntimeError("This Linspector Suite host does not support coverage annotations.")
        if not callable(provider):
            raise TypeError("provider must be callable")
    ...
    def add_coverage_column(self, label, provider):
        ...
        if self._add_coverage_column_cb is None:
            raise RuntimeError("This Linspector Suite host does not support coverage columns.")
        label = str(label or "").strip()
        if not label:
    ...
    def get_module_states(self):
        ...
        if self._module_states_cb is None:
            raise RuntimeError("This Linspector Suite host does not expose Suite Module state.")
    ...
    def activate_applet(self, applet_id):
        ...
        if self._activate_applet_cb is None:
            raise RuntimeError("This Linspector Suite host does not support applet activation.")
    ...
    def set_navigation_provider(self, provider):
        ...
        if self._set_navigation_provider_cb is None:
            raise RuntimeError("This Linspector Suite host does not support navigation organizers.")
...
class ModuleRegistry:
    ...
    def _load_entry_module(self, manifest):
        package_name = "linspector_suite_module_" + re.sub(
    ...
    def load_instance(self, manifest, context):
        if manifest.module_api not in SUPPORTED_MODULE_APIS:
            ...
            raise RuntimeError(
                ...
                f"this Linspector supports API {supported}."
            )
