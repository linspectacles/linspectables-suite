...
class ConfigurationDialog(QDialog):
    ...
    def __init__(
        ...
        self.dynamic_pages = []
        self.changed = False
        self.setWindowTitle("Configuration — Linspector Suite")
        self.resize(820, 650)
        ...
        note = QLabel(
            ...
            "When disabled, enabled applets are initialized during Linspector Suite startup. "
        ...
        modules_note = QLabel(
            "This is an offline catalogue and manager for Linspector Suite Modules. Installed modules can be "
    ...
    def add_applet(self):
        filename, _ = QFileDialog.getOpenFileName(
            self,
            "Add Linspector Applet",
            "",
            "Linspector Applet (*.zip);;ZIP Archives (*.zip)",
        )
        if not filename:
            return
        trust = QMessageBox.question(
            self,
            "Add Applet",
            "Linspector applets contain executable Python code. Only add an applet you trust.\n\nContinue with this archive?",
    ...
    def remove_applet(self):
        ...
        answer = QMessageBox.question(
            self,
            "Remove Applet",
            f"Remove '{manifest.name}' from this portable Linspector Suite copy?\n\n"
    ...
    def add_module(self):
        filename, _ = QFileDialog.getOpenFileName(
            self,
            "Add Linspector Suite Module",
            "",
            "Linspector Suite Module (*.zip);;ZIP Archives (*.zip)",
        )
        if not filename:
            return
        trust = QMessageBox.question(
            self,
            "Add Suite Module",
            "Suite Modules contain executable Python code that runs inside the Linspector Suite host when enabled. "
...
class AboutDialog(QDialog):
    ...
    def __init__(self, program_root, parent=None):
        ...
        self.setWindowTitle("About — Linspector Suite")
        ...
        logo_path = self.program_root / "assets" / "linspector-logo.png"
        ...
        name = QLabel("<h2 style='margin:0'>Linspector Suite</h2>")
        name.setAlignment(Qt.AlignmentFlag.AlignCenter)
        root.addWidget(name)

        subtitle = QLabel("<b>Linux Inspection Utilities</b>")
        ...
        details = QLabel(
            ...
            f"{self.program_root / 'config' / 'linspector.json'}"
        )
        ...
        links = QLabel(
            "<a href='https://github.com/linspector-suite/'>GitHub</a> &nbsp;·&nbsp; "
...
class LinspectorSuite(QMainWindow):
    def __init__(self, program_root):
        ...
        icon_path = self.program_root / "assets" / "linspector-icon.png"
    ...
    def setup_menu(self):
        ...
        repository_action.triggered.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://github.com/linspector-suite/linspector-suite")
            )
        )
        ...
        issue_action.triggered.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://github.com/linspector-suite/linspector-suite/issues/new")
            )
        )
        ...
        about_action = QAction("About Linspector Suite", self)
    ...
    def setup_content_area(self):
        ...
        subtitle = QLabel("Linux Inspection Utilities")
    ...
    def _add_module_menu_action(self, module_id, menu_id, text, callback, shortcut=None):
        ...
        action.setProperty("linspectorModuleId", str(module_id))
    ...
    def _reorder_module_dashboard_widgets(self):
        ...
        widgets.sort(key=lambda w: self._module_rank(w.property("linspectorModuleId") or ""))
        visible_widgets = 0
        for widget in widgets:
            self.dashboard_module_layout.removeWidget(widget)
        for widget in widgets:
            module_id = str(widget.property("linspectorModuleId") or "")
            visible = self._module_dashboard_visible(module_id)
            widget.setVisible(visible)
            if visible:
                visible_widgets += 1
            self.dashboard_module_layout.addWidget(
                widget, int(widget.property("linspectorModuleStretch") or 0)
            )
    ...
    def _add_module_dashboard_widget(self, module_id, widget, stretch=0):
        ...
        widget.setProperty("linspectorModuleId", str(module_id))
        widget.setProperty("linspectorModuleStretch", int(stretch))
...
def run(program_root):
    ...
    app.setApplicationName("Linspector Suite")
    app.setOrganizationName(ORGANIZATION_ID)
    icon_path = Path(program_root).resolve() / "assets" / "linspector-icon.png"
    if icon_path.is_file():
        app.setWindowIcon(QIcon(str(icon_path)))
    suite = LinspectorSuite(program_root)
    suite.show()
    return app.exec()
